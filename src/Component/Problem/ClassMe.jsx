...
const ClassMe = () => {
  ...
  const liveClass = useMemo(() => {
    ...
    const filtered = list.some((c) => c?.type !== undefined)
      ? list.filter((c) => String(c?.type) === "3" || c?.type === 1)
      : list;
    ...
    return filtered.map((c) => {
      ...
      return {
        ...
        type: "Problem Solving", // since type=3
      };
    });
  }, [allClass]);
                        ...
                        <div className="mt-3 flex justify-end">
                          <button
                            onClick={(e) => {
                              e.preventDefault();
                              navigate(
                                `/update-problem-class/${ev.subject_id}`
                              );
                            }}
