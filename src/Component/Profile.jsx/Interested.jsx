...
import { FaShapes, FaHandshake, FaMoneyBillWave } from "react-icons/fa";
...
const Card = ({ course, type }) => {
  ...
  const getDetailsRoute = () => {
    switch (Number(type)) {
      case 1:
        return `/details-record/${course.subject_id}`;
      case 3:
        return `/details-problem/${course.subject_id}`;
      default:
        return `/details-live/${course.subject_id}`;
    }
  };
        ...
        <div className="p-4">
          ...
          <div className="mt-3 flex items-center justify-between text-[13px] text-gray-700">
            ...
            <span className="flex items-center gap-2">
              ...
              <span className="font-semibold text-purple-600">Negotiating</span>
            </span>
          </div>
          ...
          <div className="mt-2 flex items-center justify-between text-[13px]">
            <div className="flex flex-col gap-1">
              ...
              <span className="font-bold text-green-600">
                BDT{" "}
                {Number(course.student_offer || 0).toLocaleString("en-US", {
            ...
            <div className="flex flex-col gap-1 text-right">
              <span className="text-gray-500">Tutor Offer:</span>
              <span className="font-bold text-blue-600">
                BDT{" "}
                {Number(course.tutor_offer || 0).toLocaleString("en-US", {
                  minimumFractionDigits: 2,
                  maximumFractionDigits: 2,
                })}
              </span>
...
const Negotiation = () => {
  ...
  const getPageTitle = () => {
    switch (Number(type)) {
      case 1:
        return "My Record Class Negotiations";
      case 3:
        return "My Problem Solving Negotiations";
      default:
        return "My Live Class Negotiations";
    }
  };
  ...
  const fetchNegotiations = async (page = 1) => {
    setLoading(true);
    setError(null);
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error(
          `HTTP ${response.status}: Failed to fetch negotiations`
        );
      }
        ...
        <div className="max-w-6xl mx-auto">
          ...
          <div className="mb-6 px-2">
            <div className="flex items-center gap-3 mb-2">
              <div className="inline-flex items-center justify-center w-12 h-12 rounded-full bg-gradient-to-br from-purple-600 to-pink-600 shadow-lg">
                <FaHandshake className="text-2xl text-white" />
              </div>
              <h3 className="text-3xl font-bold mb-0">{getPageTitle()}</h3>
            </div>
            <p className="text-gray-600 ml-15">
              Track your ongoing price negotiations with course instructors
            </p>
            <div className="mt-1">
          ...
          {loading ? (
            ...
            <>
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
                {negotiations.map((nego) => (
                  <Card key={nego.id} course={nego} type={type} />
                ))}
              </div>
            ...
            <div className="text-center py-16 bg-white rounded-2xl shadow-xl">
              ...
              <p className="text-gray-600 mb-6">
                You don't have any ongoing price negotiations. Browse courses
