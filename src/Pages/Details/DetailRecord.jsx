...
/** Convert watch/short URLs to embeddable URLs (fallback to original) */
...
/** Open episode in a clean new tab with an embedded player (not youtube site) */
const openEpisodeInNewTab = (ep = {}) => {
  try {
    const base = toEmbedURL(ep?.url || "");
    // If it's a YouTube embed, append origin & sensible params
    const isYT = /youtube\.com\/embed|youtu\.be/i.test(base);
    const src = isYT
      ? `${base}${
          base.includes("?") ? "&" : "?"
        }rel=0&modestbranding=1&playsinline=1&origin=${encodeURIComponent(
          window.location.origin
        )}`
      : base;

    // IMPORTANT: no noopener/noreferrer so about:blank inherits our origin
    const w = window.open("", "_blank");
    if (!w) return;

    const safeTitle = String(ep?.title || "Episode")
      .replace(/</g, "&lt;")
      .replace(/>/g, "&gt;");
...
<head>
  ...
  <style>
    ...
    header{padding:12px 16px;font-family:system-ui,-apple-system,Segoe UI,Roboto,Ubuntu,Cantarell,Noto Sans,sans-serif;border-bottom:1px solid #eee}
    .player{flex:1}
...
<body>
  <div class="wrap">
    ...
    // Fallback: still avoid YouTube site if possible, but at least open the URL
    const src = toEmbedURL(ep?.url || "");
...
const DEFAULT_CLASS_TYPE = 3; // recorded class identifier for negotiation endpoints
...
const normalizeClass = (payload) => {
  ...
  console.log("Normalized duration1:", duration1);
  ...
  return {
    ...
    buyers: Array.isArray(r.buyers) ? r.buyers : r.buyers || [], // keep as-is if present
...
/** -----------------------------
 * NEW: Fetch episodes from courseAllItemByCourseID
 * ------------------------------*/
async function fetchEpisodesByCourseId(courseId, signal) {
  ...
  return list
    .map((ep, index) => ({
      ...
      minutes:
        typeof ep.time_duration === "number"
          ? ep.time_duration
          : Number(ep.time_duration) ||
            getEpisodeMinutes({ duration: ep.time_duration }),
      url: ep.url || "",
      isPaid: ep.isPaid,
      hasEnded: ep.has_ended,
      position:
        typeof ep.position === "number"
          ? ep.position
          : Number(ep.position) || index,
      createdAt: ep.create_time,
    }))
    .sort(
      (a, b) =>
...
const DetailsRecord = () => {
  ...
  /** -----------------------------
   * NEW: episodes state (from the new endpoint)
   * ------------------------------*/
  ...
  /** -----------------------------
   * NEW: fetch episodes when subject_id changes
   * ------------------------------*/
  useEffect(() => {
    let isMounted = true;
    const controller = new AbortController();
    ...
    (async () => {
      try {
        if (!subject_id) {
          setEpLoading(false);
          setEpError("No subject ID provided.");
          return;
        }
        setEpLoading(true);
        setEpError("");
        ...
        const eps = await fetchEpisodesByCourseId(
          subject_id,
          controller.signal
        );
        ...
        if (isMounted) {
          ...
          setEpError(e.message || "Failed to load episodes.");
        }
      } finally {
        if (isMounted) setEpLoading(false);
      }
    })();
  ...
  const negotiationType = Number(
    item?.classType ?? item?.type ?? DEFAULT_CLASS_TYPE
  );
  const shortDesc =
    item?.description ||
  ...
  const handleBuyNow = useCallback(async () => {
    ...
    const courseType = item?.classType ?? item?.type ?? 1;
  ...
  }, [courseId, isFree, item?.classType, item?.status, item?.type]);
  ...
  const currentUser = (() => {
    const authData = JSON.parse(localStorage.getItem("auth") || "{}");
    return authData?.user?.full_name || authData?.user?.name || "Anonymous";
  })();
  ...
  const fetchReviews = async (courseId, rating = null) => {
    ...
    try {
      const url = rating
        ? `${Base_url}getFilterCourseFeedback?courseId=${encodeURIComponent(
            courseId
          )}&rating=${rating}`
  ...
  const submitReview = async (courseId, comment, rating) => {
    ...
    formData.append("courseType", "1"); // recorded class type
  ...
  // Fetch reviews on component mount and when rating filter changes
  useEffect(() => {
    if (courseId) {
          ...
          <div className="flex flex-wrap items-center justify-center gap-3 mt-4 px-4 pb-6">
            ...
            {isOwner ? (
              ...
              <button
                type="button"
                hidden
                className="px-5 py-2 rounded-2xl text-sm font-semibold text-gray-500 bg-gray-300 cursor-not-allowed shadow-sm"
              >
                Already Bought
              </button>
            ) : isFree ? (
              ...
              <div className="mt-6 space-y-4">
                ...
                <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-3">
                  ...
                  <button
                    ...
                    disabled={
                      !myNegotiation || negotiationActionId === "student:buy"
                    }
        ...
        {activeTab === "details" && (
          <div className="mt-6 flex flex-col gap-4">
            ...
            {subTab === "episodes" ? (
              <>
                <div className="mt-4">
                  <div className="rounded-2xl border border-gray-200 bg-purple-100 px-4 py-6 shadow-sm">
                    <p className="text-black mt-1">
                      ...
                      {epError && (
                        <span className="ml-2 text-red-600">{epError}</span>
                      )}
                    </p>
                  </div>
                </div>
                ...
                <div className="mt-0 flex flex-col gap-3">
                  {episodes.map((ep, idx) => {
                    const isFreeEp = Number(ep?.isPaid) === 0; // treat missing isPaid as free
                    const hasPurchasedCourse = Number(item?.status) === 1;
                    // owners or students who already purchased can click all episodes; others only free ones
                    const canClick = isOwner || hasPurchasedCourse || isFreeEp;
                    ...
                    return (
                      <div
                        ...
                        {...(canClick
                          ? {
                              onClick: () => openEpisodeInNewTab(ep),
                              onKeyDown: (e) => {
                                if (
                                  (e.key === "Enter" || e.key === " ") &&
                                  ep?.url
                                ) {
                                  openEpisodeInNewTab(ep);
                                }
                              },
                              role: "button",
                              tabIndex: 0,
                              className:
                                "flex items-center justify-between rounded-2xl bg-white border border-gray-200 px-5 py-4 shadow-sm cursor-pointer hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-purple-300",
                            }
                          : {
                              className:
                                "flex items-center justify-between rounded-2xl bg-white border border-gray-200 px-5 py-4 shadow-sm opacity-70 cursor-not-allowed",
                            })}
                        title={ep.title || `Class ${idx + 1}`}
                      >
                        <div className="flex items-center gap-4">
                          ...
                          <div className="flex flex-col">
                            ...
                            <div className="text-xs text-gray-500">
                              {`Video- ${getEpisodeMinutes(ep)} Minutes`}
                            </div>
                            {Number(ep?.isPaid) === 0 ? (
                ...
                {/* owner-only Add Episode button */}
                {isOwner && (
                  <div className="mt-2">
                    <Link to={`/add-episode/${subject_id}`} className="mt-0">
                      <button className="inline-flex items-center gap-2 rounded-2xl bg-purple-800 px-4 py-2 text-white font-semibold shadow-sm hover:brightness-105">
                        + Add Episode
                      </button>
                    </Link>
                  </div>
              ...
              <Reviews
                ...
                isOwner={isOwner}
                hasPurchased={item?.status === 1}
                hasGivenFeedback={item?.feedbackStatus === 1}
              />
            )}
          </div>
        )}
        ...
        {activeTab === "enrollbuy" && (
          <div className="mt-6 rounded-2xl border border-gray-200 bg-white p-6 text-center shadow-sm">
            ...
            <button
              ...
              disabled={buying}
              className={`px-6 py-2 rounded-2xl text-sm font-semibold text-white shadow-sm hover:brightness-105 ${
                isFree ? "bg-pink-600" : "bg-pink-600"
          ...
          >
            ...
            <div className="relative z-10 w-full max-w-4xl rounded-[28px] bg-[#ece7f0] p-6 sm:p-8">
              ...
              <div className="mt-6 grid grid-cols-2 gap-y-4 px-2 text-gray-700">
                <div className="font-medium">Total Episode</div>
                ...
                <div className="text-black font-semibold">
                  {formatTotal(Number(item?.duration || 0))}
                </div>
