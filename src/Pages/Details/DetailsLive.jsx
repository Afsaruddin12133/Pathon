...
const normalizeClass = (payload) => {
  ...
  return {
    ...
    tags: Array.isArray(tagData)
      ...
      : [],
    cover: r.headerImageVideo
      ? `https://apidocumentationpathon.pathon.app/${r.headerImageVideo}`
      : null,
...
async function fetchLiveClassData(courseId, signal) {
  ...
  const json = await res.json();
  if (json.error)
    throw new Error(json.error || "Failed to fetch live class data");
...
const DetailsLive = () => {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        const liveClassData = await fetchLiveClassData(
          courseId,
          controller.signal
        );
        if (isMounted) {
  ...
  const handleTutorOfferUpdate = async (studentId) => {
    ...
    try {
      ...
      const res = await api.post(`courseNegotiable`, body);
  ...
  const handleStudentBuy = async () => {
    try {
     ...
     // const json = await res.json().catch(() => ({}));
      if (!res.ok || res?.error) {
        throw new Error(
          res?.message || res?.error || "Failed to purchase course"
        );
      }
      ...
      <div className="w-full max-w-6xl mx-auto p-4 md:p-8">
        ...
        <div className="overflow-hidden bg-white">
          ...
          <div className="flex flex-wrap items-center justify-center gap-3 mt-4 px-4 pb-6">
            ...
            ) : item?.status === 1 ? null : isFree ? ( // User has already bought - no buttons needed, details will show automatically
              <button
                ref={enrollBtnRef}
                type="button"
