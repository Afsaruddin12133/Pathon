...
async function sendOtpToApi(phoneE164) {
  ...
  const res = await api.post(endpoint, { phone: localNumber });
    ...
    <div className="min-h-[60vh] w-full flex items-start justify-center px-4 sm:px-6 pt-24">
      <form onSubmit={verify} className="w-full max-w-md text-center">
        ...
        <p className="mt-4 text-lg text-gray-700">
          ...
          <span className="font-semibold">{phoneMasked}</span>
        </p>
...
const AuthPage = () => {
  ...
  const handleContinue = async ({
    ...
    console.log("➡️ handleContinue received:", {
      e164,
      display,
      otpSent,
      initialReqId,
  ...
  const handleResend = async () => {
    ...
    console.log(
      "✅ Parent: resend done. New requestId:",
      sent.requestId ?? null
    );
  };
