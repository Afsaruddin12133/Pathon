...
const AddAProblem = () => {
  ...
  const handleExtraFileChange = (id, e, type) => {
    ...
    if (type === "image") {
      setExtraImages((prev) =>
        prev.map((ef) => (ef.id === id ? { ...ef, file: f } : ef))
      );
    } else {
      setExtraPDFs((prev) =>
        prev.map((ef) => (ef.id === id ? { ...ef, file: f } : ef))
  ...
  const removeExtraFile = (id, type) => {
    if (type === "image")
      setExtraImages((prev) => prev.filter((ef) => ef.id !== id));
    else setExtraPDFs((prev) => prev.filter((ef) => ef.id !== id));
  };

  const addMoreFileInput = (type) => {
    if (type === "image")
      setExtraImages((prev) => [...prev, { id: Date.now(), file: null }]);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      console.log(
        "🌐 Create Problem (with files) status:",
        first.res.status,
        first.payload
      );
      ...
      if (first.res.status === 500 && cover) {
        console.warn(
          "⚠️ 500 with cover; retrying without cover (keeping attachments)..."
        );
        const second = await submitOnce(buildFormData(false));
        console.log(
          "🌐 Create Problem (no cover) status:",
          second.res.status,
          second.payload
        );
