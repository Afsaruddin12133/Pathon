    ...
    >
      ...
      >
        ...
        <p className="mt-3 text-xs text-gray-500 text-center">
          Payment options (Bkash, Visa, Master) will be shown by SSLCOMMERZ
          gateway.
        </p>
      </div>
    </div>
...
function WithdrawInputModal({ title, cta, onClose, onConfirm }) {
  ...
  const isValid =
    amount.trim() !== "" && method.trim() !== "" && number.trim() !== "";
    ...
    >
      ...
      >
        ...
        <button
          ...
          onClick={() =>
            isValid && onConfirm(amount.trim(), method.trim(), number.trim())
          }
        >
          {cta}
        </button>
...
const SuccessShell = ({ title, body, cta = "BACK TO HOME", onClose }) => (
  ...
  >
    <div
      className="w-[92vw] max-w-2xl rounded-3xl bg-[#F5F8FF] shadow-xl p-6 sm:p-10 relative"
      onClick={(e) => e.stopPropagation()}
    >
      <button
        onClick={onClose}
      ...
      <div className="flex justify-center">
        <img
          src="/src/Images/thank-you.svg"
          alt="Success Illustration"
          className="h-36 sm:h-44 object-contain select-none pointer-events-none"
        />
      </div>
      <h3 className="mt-4 sm:mt-6 text-center text-[20px] sm:text-[22px] font-extrabold text-gray-900">
        {title}
      </h3>
      {body && (
        <p className="mt-2 text-center text-gray-600 leading-relaxed max-w-xl mx-auto">
          {body}
        </p>
      )}
      <div className="mt-6 sm:mt-8 flex justify-center">
        <button
          onClick={onClose}
          className="w-full sm:w-3/4 rounded-md bg-purple-600 hover:bg-purple-700 text-white font-extrabold tracking-wide py-3"
        >
          {cta}
        </button>
      </div>
...
const Transaction = () => {
  ...
  useEffect(() => {
    const fetchProfile = async () => {
      try {
        ...
        setBalanceDisplay(
          Number.isFinite(fetchedBalance) ? fetchedBalance.toFixed(2) : "0"
        );
      } catch (error) {
  ...
  const loadTransactions = useCallback(async (pageToLoad = 1, query = "") => {
    ...
    try {
      const url = query.trim()
        ? `${Base_url}getUserFilterAllTransaction?query=${encodeURIComponent(
            query.trim()
          )}&page=${pageToLoad}`
  ...
  const transactionRows = useMemo(() => {
    return transactions.map((item, index) => {
      ...
      const prevCumulativeRaw = Number(
        transactions?.[index + 1]?.cumulative_balance
      );
      const prevCumulative = Number.isFinite(prevCumulativeRaw)
        ? prevCumulativeRaw
        : null;
        ...
        <div className="mt-6 flex flex-col items-center">
          <div className="relative">
            <img
              src={displayAvatar}
              alt="User avatar"
              className="h-28 w-28 sm:h-32 sm:w-32 rounded-full object-cover ring-4 ring-white shadow"
            />
            <span className="absolute inset-0 rounded-full ring-2 ring-purple-200 pointer-events-none" />
          </div>
          <p className="mt-3 text-md lg:text-lg font-semibold text-gray-800">
            {userDisplayName}
          </p>
        </div>
            ...
            <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-4">
              <div className="flex flex-col sm:flex-row items-center justify-between gap-2">
                ...
                <div className="mt-4 flex flex-col sm:flex-row items-center justify-center gap-2 sm:gap-6">
                  <button
                    className="px-3 py-1.5 sm:py-2 rounded-md bg-purple-600 text-white text-sm font-semibold hover:bg-purple-700"
                    onClick={() => setShowAdd(true)}
                  >
                    Add Balance
                  </button>
                  <button
                    className="px-3 py-1.5 sm:py-2 rounded-md bg-violet-600 text-white text-sm font-semibold hover:bg-violet-700"
          ...
          <div className="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
                  ...
                  <tbody className="divide-y divide-gray-100">
                    {transactionRows.map((row) => (
                      <tr key={row.id} className="hover:bg-gray-50">
                          ...
                          {row.fromToUserId ? (
                            <Link
                              to={{
                                ...
                                search: `?user_id=${encodeURIComponent(
                                  row.fromToUserId
                                )}`,
                              }}
                        ...
                        <td className="px-4 sm:px-6 py-3 text-green-600 font-semibold text-right whitespace-nowrap">
                          {row.inAmount}
                        </td>
                        <td className="px-4 sm:px-6 py-3 text-red-500 font-semibold text-right whitespace-nowrap">
                          {row.outAmount}
                        </td>
                        <td className="px-4 sm:px-6 py-3 text-gray-800 font-semibold text-right whitespace-nowrap">
                          {row.balance}
                        </td>
                      </tr>
                    ))}
                    ...
                    {transactionsError && (
                      <tr>
                        <td
                          colSpan={6}
                          className="px-4 sm:px-6 py-6 text-center text-sm text-red-500"
                        >
                          {transactionsError}
                        </td>
                      </tr>
                    ...
                    {noTransactions && !transactionsError && (
                      <tr>
                        <td
                          colSpan={6}
                          className="px-4 sm:px-6 py-10 text-center text-sm text-gray-500"
                        >
            ...
            <div className="px-4 sm:px-6 py-5 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 border-t border-gray-100">
              <div className="text-xs text-gray-500">
                Page{" "}
                {viewMode === "withdraws" ? withdrawsPage : transactionsPage}
                {viewMode === "withdraws"
                  ? withdrawsTotal
                    ? ` of ${Math.max(Math.ceil(withdrawsTotal / 10), 1)}`
                    : ""
                  : transactionsTotal
                  ? ` of ${Math.max(Math.ceil(transactionsTotal / 10), 1)}`
                  : ""}
              </div>
              {viewMode === "withdraws" ? (
                withdrawsHasMore ? (
                  <button
                    ...
                    className={`w-full sm:w-auto inline-flex items-center justify-center rounded-md border border-purple-500 px-4 py-2 text-sm font-semibold transition ${
                      withdrawsLoading
