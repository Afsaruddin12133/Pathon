...
const fieldDefinitions = [
  ...
  },
  {
    ...
    type: "text",
    editable: true,
    apiKey: "educational_qualification",
    inline: true,
  },
  {
    key: "profession",
  ...
  const saveValue = async (newVal) => {
    ...
    if (key === "interests") {
      try {
        ...
        // revert UI
        setRows((prev) =>
          prev.map((r) => (r.key === "interests" ? { ...r, value: oldVal } : r))
        );
    ...
    img.onload = () => {
      ...
      (async () => {
        try {
          ...
          if (isSuccess) {
            ...
            if (maybeUrl) {
              ...
              check.onload = () => {
                ...
                console.info(
                  "Profile picture updated successfully with server URL"
                );
              };
              check.onerror = () => {
                console.warn(
                  "Uploaded image URL failed to load, keeping blob preview"
                );
              ...
              console.info(
                "Profile picture updated successfully (no URL returned, keeping blob)"
              );
            }
          } else {
          ...
          alert(
            "Couldn't update profile picture. Please try again. Error: " +
              err.message
          );
        }
      })();
  ...
  useEffect(() => {
    const initFromApi = async () => {
      try {
        ...
        const userId = safeGet(
          auth,
          ["user.id", "data.user.id", "id", "user_id", "data.user_id"],
          null
        );
        ...
        const token = safeGet(
          auth,
          ["user.token", "data.user.token", "token"],
          null
        );
        ...
        const interests = Array.isArray(payload?.interested)
          ? payload.interested
              .map((i) => (i?.title ?? "").trim())
              .filter(Boolean)
              .join(", ")
          ...
          const value = (() => {
            switch (field.key) {
              ...
              case "dob":
                return (
                  normalizeDOBToDMY(
                    apiUser.dob ?? apiUser.date_of_birth ?? ""
                  ) || "N/A"
                );
              case "address":
              ...
              case "education":
                return (
                  apiUser.educational_qualification ??
                  apiUser.education ??
                  apiUser.degree ??
                  "N/A"
                );
