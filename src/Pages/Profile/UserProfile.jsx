  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        setCourses({
          preRecordData: Array.isArray(data?.items?.preRecordData)
            ? data.items.preRecordData
            : [],
          liveCountData: Array.isArray(data?.items?.liveCountData)
            ? data.items.liveCountData
            : [],
          problemSolvingData: Array.isArray(data?.items?.problemSolvingData)
            ? data.items.problemSolvingData
            : [],
        });
      } catch (err) {
          ...
          <div>
            ...
            {requestedUserId && (
              <p className="text-sm text-gray-500">
                User ID:{" "}
                <span className="font-semibold">{requestedUserId}</span>
              </p>
            )}
          </div>
        </div>
              ...
              <div className="flex-1 space-y-4">
                ...
                <div className="grid gap-3 md:grid-cols-2">
                  {[...Array(6)].map((_, idx) => (
                    <div
                      key={idx}
                      className="rounded-2xl border border-gray-200 bg-white/80 px-4 py-3 shadow-sm"
                    >
            ...
            <section className="flex flex-col gap-6 rounded-3xl border border-gray-200 bg-white p-6 shadow-sm lg:flex-row">
              ...
              <div className="flex-1 space-y-4">
                <div className="grid gap-3 md:grid-cols-2">
                  <ProfileRow
                    label="Role"
                    value={profile.role === 1 ? "Tutor" : "Student"}
                  />
            ...
            <section className="rounded-3xl border border-gray-200 bg-white p-6 shadow-sm">
              <div className="flex flex-wrap items-center justify-between gap-3">
                ...
                <span className="text-sm text-gray-500">
                  {interestTags.length} item
                  {interestTags.length === 1 ? "" : "s"}
                </span>
              </div>
